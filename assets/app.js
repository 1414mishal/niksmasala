...
/* Newsletter handler — persists to Supabase so a real list exists.
   TODO(backend): wire this to your email provider (Mailchimp / Resend audience). */
async function handleNewsletter(e){
  ...
  try{
    ...
    await db.from('newsletter').upsert({email,created_at:new Date().toISOString()},{onConflict:'email'});
  }catch(_){
    /* fallback to local so we don't lose it during the demo */
